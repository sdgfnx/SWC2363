...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['LastName'])) {
        ...
        $l = mysqli_real_escape_string($connect, trim($_POST['LastName'])); // Fix: changed "$1" to "$l"
    }
    ...
    if (empty($error)) {
        $q = "SELECT ID FROM doktor WHERE Specialization = '$s' AND ID != $id"; // Fix: added "=" for Specialization

        $result = @mysqli_query($connect, $q);

        if (mysqli_num_rows($result) == 0) {
            $q = "UPDATE doktor SET FirstName='$n', LastName='$l', Specialization='$s', Password='$p' WHERE ID='$id' LIMIT 1"; // Fix: corrected query
