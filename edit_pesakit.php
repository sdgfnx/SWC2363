...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['LastName_P'])) {
        ...
        $l = mysqli_real_escape_string($connect, trim($_POST['LastName_P'])); // Corrected variable from $1 to $l
    }
    ...
    if (empty($error)) {
        ...
        if(mysqli_num_rows($result) == 0) {
            $q = "UPDATE pesakit SET FirstName_P='$n', LastName_P='$l', Diagnose='$d' WHERE ID_P='$id' LIMIT 1";
            $result = @mysqli_query($connect, $q);
